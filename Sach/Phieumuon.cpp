...
void PhieuMuon::update_pm(PhieuMuon arr[1000], int n)
{
    ofstream file_s("../PhieuMuon.txt");
    for (int i = 0; i < n; i++) {
        file_s << arr[i].soPhieuMuon << "," << arr[i].maSach << "," << arr[i].maBanDoc << "," << arr[i].ngayMuon << "," << arr[i].ngayTra << "," << arr[i].tinhTrang << endl;
    }
    file_s.close();
}

void PhieuMuon::title_phieumuon()
{
    int w = 25;
...
void PhieuMuon::getlist_pm(PhieuMuon arr[1000], int& sl)
{
    fstream f;
    int i = 0;
    sl = 0;
